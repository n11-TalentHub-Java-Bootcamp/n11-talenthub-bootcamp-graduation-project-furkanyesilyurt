...
public class CalculateCreditLimitIncomeBelow5000 implements CreditLimitStrategy{

    @Override
    public LoanResponseDto getCreditLimit(Double guarantee, Double monthlyIncome) {
        Double totalLimit;
        totalLimit = IncomeBelow5000.default_credit_limit.getValue();

        if (guarantee != null) {
            totalLimit += guarantee * IncomeBelow5000.guarantee_percent.getValue();
        }
